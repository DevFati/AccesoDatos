...
public class OperacionesEmple {
	public boolean comprobarDep(Connection conexion, int id) {
	...
	public String modificaremple(Connection conexion, int empno, String ape, String ofi, float sal, float comi,
			java.sql.Date fecha, int dep, int dir) {
		String mensaje = "";

		if (comprobarEmple(conexion, empno)) {
			// existe, se puede modificar
			if (comprobarDep(conexion, dep)) {
				...
				mensaje = "DEPARTAMENTO NO EXISTE: " + dep + ", NO SE ACTUALIZA";
			}
	...
	public String insertarEmple(Connection conexion, int empno, String ape, String ofi, float sal, float comi,
			java.sql.Date fecha, int dep, int dir) {
		String mensaje = "";
	...
	public static void verempleados(Connection conexion) {
		...
		try {
			...
			ResultSet resul = sentencia.executeQuery();
			System.out.printf("%10s %15s %15s %10s %10s %10s %10s %10s %n", "EMP_NO", "APELLIDO", "OFICIO", "DIRECCION",
					"FECHA ALTA ", " SALARIO", "COMISION", "DEPARTAMENTO");
